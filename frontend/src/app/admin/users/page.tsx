...
export default function AdminUsersPage() {
  ...
  const getRoleBadge = (role: string) => {
    const roleMap: Record<string, { bg: string; label: string }> = {
      researcher: { bg: 'bg-[#3B82F6]', label: 'RESEARCHER' },
      organization: { bg: 'bg-[#10B981]', label: 'ORGANIZATION' },
      ...
      {user ? (
        ...
        >
          ...
          <div className="mt-6 flex gap-2 flex-wrap">
            ...
            <Link href="/admin/users?role=researcher">
              <Button 
                variant={roleFilter === 'researcher' ? 'primary' : 'outline'} 
                size="sm"
                className={roleFilter === 'researcher' ? 'bg-[#EF2330] hover:bg-[#DC2026]' : ''}
              >
                Researchers
              </Button>
            </Link>
            <Link href="/admin/users?role=organization">
              <Button 
                variant={roleFilter === 'organization' ? 'primary' : 'outline'} 
                size="sm"
                className={roleFilter === 'organization' ? 'bg-[#EF2330] hover:bg-[#DC2026]' : ''}
              >
                Organizations
              </Button>
            </Link>
            <Link href="/admin/users?role=triage_specialist">
              <Button 
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-2">
                          ...
                          <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${
                            u.status === 'active' ? 'bg-[#10B981]' : 'bg-[#94A3B8]'
                          } text-white`}>
